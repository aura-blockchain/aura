...
# Add to crontab: 0 3 * * * /home/hudson/blockchain-projects/aura/scripts/backup/validator-backup.sh aura aura-testnet-artifacts >> /var/log/aura-backup.log 2>&1

if [ "$1" == "--install" ]; then
    (crontab -l 2>/dev/null | grep -v "validator-backup.sh"; echo "0 3 * * * /home/hudson/blockchain-projects/aura/scripts/backup/validator-backup.sh aura aura-testnet-artifacts >> ~/.validator-backups/backup.log 2>&1") | crontab -
    echo "Cron job installed for daily 3 AM backups"
else
