#!/bin/bash
# ============================================================================
# AURA Testnet Health Check
# ============================================================================
# Checks both AURA testnet nodes (primary on aura-testnet, secondary on
...
# ============================================================================
# Check Primary Node (aura-testnet:10657)
# ============================================================================
check_primary() {
    log "${BLUE}Primary Node (aura-testnet)${NC}"

    local status
    status=$(ssh -o ConnectTimeout=5 aura-testnet "curl -s http://127.0.0.1:10657/status" 2>/dev/null)
    ...
    local net_info
    net_info=$(ssh -o ConnectTimeout=5 aura-testnet "curl -s http://127.0.0.1:10657/net_info" 2>/dev/null)
...
# ============================================================================
# Check Secondary Node (services-testnet:26657)
# ============================================================================
check_secondary() {
    log "${BLUE}Secondary Node (services-testnet)${NC}"

    local status
    status=$(ssh -o ConnectTimeout=5 services-testnet "curl -s http://127.0.0.1:26657/status" 2>/dev/null)
    ...
    local net_info
    net_info=$(ssh -o ConnectTimeout=5 services-testnet "curl -s http://127.0.0.1:26657/net_info" 2>/dev/null)
...
main() {
    log ""
    log "${BOLD}AURA Testnet Health Check - $(date)${NC}"
    log ""
    ...
    if [ "$JSON_OUTPUT" = true ]; then
        ...
        if [ "$PRIMARY_OK" = true ] && [ "$SECONDARY_OK" = true ] && [ "$PEERED" = true ]; then
            log "${GREEN}${BOLD}AURA Testnet: HEALTHY${NC}"
            log "Height: ${PRIMARY_HEIGHT} blocks, 2 nodes peered"
            exit 0
        else
            log "${RED}${BOLD}AURA Testnet: ISSUES DETECTED${NC}"
            exit 1
        fi
    fi
}
