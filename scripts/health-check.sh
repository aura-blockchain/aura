#!/bin/bash
# ============================================================================
# AURA Blockchain - Node Health Check
# ============================================================================
# Lightweight health checker for a single AURA node.
# Probes /status, /net_info, /validators and fails fast on errors.
#
# Usage: ./health-check.sh
#        RPC_ENDPOINT=http://10.10.0.1:10657 ./health-check.sh
# ============================================================================

set -euo pipefail

RPC_ENDPOINT="${RPC_ENDPOINT:-http://127.0.0.1:10657}"
...
if [[ "${catching_up}" != "false" ]]; then
    echo "node is still catching up at height ${latest_height}"
    exit 2
fi
...
echo "ok moniker=${moniker} height=${latest_height} peers=${peer_count} validators=${validator_set_count}"
