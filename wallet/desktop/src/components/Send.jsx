...
const Send = ({ walletData, onSuccess }) => {
  ...
  const validateForm = () => {
    ...
    if (!recipient.startsWith('aura')) {
      throw new Error('Invalid recipient address (must start with "aura")');
    }
  ...
  const handleSend = async () => {
    try {
      ...
      // Convert amount to smallest unit (uaura)
      const amountInUaura = Math.floor(parseFloat(amount) * 1000000);
  ...
  return (
    <div className="content">
      <div className="card" style={{ maxWidth: '600px', margin: '0 auto' }}>
        <h3 className="card-header">Send Aura</h3>
        ...
        <div className="form-group">
          <label className="form-label">Amount (Aura)</label>
          <input
            type="number"
            className="form-input"
