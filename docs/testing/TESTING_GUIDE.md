...
#### DEX Swap Telemetry Scenario

`TestDEXSwapWorkflow` (in `chain/testing/integration/integration_test.go`) spins up an in-memory DEX keeper, creates a pool/order, records a swap, and then exercises the gRPC query server so `user-orders` and `market-price` reflect the persisted telemetry. Run it with:

```bash
cd chain
go test ./testing/integration -run TestDEXSwapWorkflow -v
```

This mirrors the CLI workflow operators follow on a real node: submit liquidity, execute a swap, then confirm `aurad query dex user-orders` and `aurad query dex market-price` show the expected data.
