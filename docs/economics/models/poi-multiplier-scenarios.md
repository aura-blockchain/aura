...
## Assumptions
- Treasury allocation: **200,000,000 AURA** dedicated to PoI payouts.
- Base reward per verified human: **6 AURA** (covers verifier fee rebate + participation stipend). Adjust once fee data exists.
...
| Range | Users Covered | Multiplier | Payout/User (AURA) | Treasury Draw | Treasury Left |
| ----- | ------------- | ---------- | ----------------- | ------------- | ------------- |
...
Branching past 45M verified users still leaves >36M AURA (18% of treasury) for future halvings. The CSV version (`poi-multiplier-scenarios.csv`) can seed notebooks that simulate various verification rates and fee offsets.

## Next Steps
1. Validate the **6 AURA base reward** against expected verifier costs; update both CSV + RFC if it changes.
