# Aura Testnet Prelaunch Runbook (Parallel-Safe, Extensive)
...
- Set `RUN_ID` to a unique value per agent (e.g., `aura-agent1-YYYYMMDDHHMMSS`).
...
### 0.3 Local devnet functional scripts (do not run on public testnet)

These scripts expect local docker-based validators. Run them on a local devnet to catch module regressions:

```bash
bash test-scripts/01-test-multisend.sh
bash test-scripts/02-test-staking.sh
bash test-scripts/03-test-governance.sh
bash test-scripts/04-test-security.sh
bash test-scripts/05-test-wasm.sh
```

## Phase 1: Parallel-Safe Testnet Suite

These steps are safe to run in parallel with PAW and XAI. They are read-only or minimal-impact.
...
### 2.1 Staking lifecycle

- Delegate to a validator, then unbond (small amounts).
- Verify staking queries and balances post-unbond.

### 2.2 Governance lifecycle

- Submit a text proposal (low deposit), deposit, vote, and verify tally.

### 2.3 WASM lifecycle (if enabled)

- Store contract, instantiate, execute, query, and migrate.
...
### 2.5 High load / mempool stress

- Use high-throughput load tests only during a window.
