# AURA Testnet Faucet - Testing Summary
...
**Note**: E2E tests require PostgreSQL, Redis, and optionally a AURA node. They will be skipped if dependencies are not available.
...
3. **AURA Node** (optional, for blockchain interaction tests)
   ```bash
...
3. **Blockchain Integration Tests**: May show warnings if AURA node is not running (expected)

### Expected Warnings
- Integration tests may fail on blockchain calls if a AURA node is not running - this is expected and documented
...
The AURA Testnet Faucet has comprehensive test coverage with:
