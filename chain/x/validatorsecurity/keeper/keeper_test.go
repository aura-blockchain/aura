...
func (suite *KeeperTestSuite) TestDoubleSignEvidence() {
	...
	// Retrieve evidence
	retrieved, found := suite.keeper.GetDoubleSignEvidence(suite.ctx, validatorAddr, 100)
...
func (suite *KeeperTestSuite) TestDowntimeInfraction() {
	...
	// Retrieve infraction
	retrieved, found := suite.keeper.GetDowntimeInfraction(suite.ctx, validatorAddr)
	suite.Require().True(found)
	suite.Require().Equal(validatorAddr, retrieved.ValidatorAddress)
