...
func setupModule(t *testing.T) (AppModule, *keeper.Keeper, sdk.Context, codec.JSONCodec) {
	...
	k := keeper.NewKeeper(binaryCodec, storeService, paramsStore, "authority")
...
func TestAppModuleBasic_ValidateGenesis(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:    "empty genesis",
			genesis: json.RawMessage(""),
			wantErr: true, // economicsecurity module requires valid genesis JSON
		},
		{
			name:    "invalid json",
...
func TestNewAppModule(t *testing.T) {
	...
	storeService := runtime.NewKVStoreService(storeKey)
	paramsStore := params.NewStore(*types.DefaultParams())
	k := keeper.NewKeeper(cdc, storeService, paramsStore, "authority")
