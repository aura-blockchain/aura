...
func setupModule(t *testing.T) (AppModule, *keeper.Keeper, sdk.Context, codec.JSONCodec) {
	...
	k := keeper.NewKeeper(storeKey, binaryCodec, "authority")
...
func TestAppModuleBasic_ValidateGenesis(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:    "empty genesis",
			genesis: json.RawMessage(""),
			wantErr: true, // contractregistry module requires valid genesis JSON
		},
		{
			name:    "invalid json",
...
func TestNewAppModule(t *testing.T) {
	...
	interfaceRegistry := codectypes.NewInterfaceRegistry()
	cdc := codec.NewProtoCodec(interfaceRegistry)
	k := keeper.NewKeeper(storeKey, cdc, "authority")
