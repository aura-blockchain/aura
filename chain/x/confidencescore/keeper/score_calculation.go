...
func (k *Keeper) CalculateScoreEarned(walletAddr, irID string) (uint64, float32, float32, float32, error) {
	...
	velocityBonus := k.CalculateVelocityBonus(walletAddr)
	arenaBonus := k.CalculateArenaMultiplier(walletAddr, arena)
	jackpotBonus := k.CheckJackpotWin(walletAddr, irID)
...
func (k *Keeper) CalculateArenaMultiplier(walletAddr, arena string) float32 {
	if arena == "" {
		return 1.0
	}
...
func (k *Keeper) CalculateTotalScore(walletAddr string) (uint64, error) {
	record, ok := k.GetUserRecord(walletAddr)
	if !ok {
...
func (k *Keeper) CalculateArenaScore(walletAddr, arena string) (uint64, error) {
	record, ok := k.GetUserRecord(walletAddr)
	if !ok {
		return 0, types.ErrUserRecordNotFound
	}
...
func (k *Keeper) RecalculateScore(walletAddr string) (uint64, uint64, []string, error) {
	record, ok := k.GetUserRecord(walletAddr)
	if !ok {
	...
	calculatedTotal, err := k.CalculateTotalScore(walletAddr)
	if err != nil {
		return 0, 0, nil, err
	}
...
func (k *Keeper) CheckVerificationStatus(walletAddr string) (types.VerificationStatus, error) {
	record, ok := k.GetUserRecord(walletAddr)
	if !ok {
...
func (k *Keeper) ApplyArenaFocusBonus(walletAddr string) ([]string, error) {
	record, ok := k.GetUserRecord(walletAddr)
	if !ok {
...
func (k *Keeper) GetArenaBreakdown(walletAddr string) (map[string]types.ArenaScore, []string, error) {
	record, ok := k.GetUserRecord(walletAddr)
	if !ok {
