...
func (k *Keeper) CalculatePoIReward(auraPrice math.LegacyDec) math.Int {
...
func (k *Keeper) DistributePoIReward(
	...
	totalReward := k.CalculatePoIReward(auraPrice)
...
func (k *Keeper) GetCurrentRewardAmount(auraPrice math.LegacyDec) math.Int {
	return k.CalculatePoIReward(auraPrice)
}
...
func (k *Keeper) GetRewardTierInfo(auraPrice math.LegacyDec) (tierName string, rewardAmount math.Int) {
	if auraPrice.LT(math.LegacyNewDecWithPrec(11, 2)) {
		...
		reward := k.CalculatePoIReward(auraPrice)
		return "Established (≥$0.50)", reward
	}
}

// GetTotalRewardsDistributed returns total PoI rewards distributed to a user
func (k *Keeper) GetTotalRewardsDistributed(ctx sdk.Context, walletAddr string) uint64 {
	// In a full implementation, sum all reward amounts from IR completions
	// For now, return 0 as reward tracking would need to be added to the proto
	return 0
}
