...
func (suite *KeeperTestSuite) SetupTest() {
	...
	db := store.NewCommitMultiStore(nil, log.NewNopLogger(), metrics.NewNoOpMetrics())
...
func (suite *KeeperTestSuite) TestCheckSpendingLimit_Exceeded() {
	...
	suite.Require().Error(err)
	suite.Require().Equal(types.ErrSpendingLimitExceeded, err)
}
...
func (suite *KeeperTestSuite) TestCheckDustTransaction_Blocked() {
	...
	// Check transaction below minimum
