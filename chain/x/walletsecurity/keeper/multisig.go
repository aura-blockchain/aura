...
func (k Keeper) CreateMultiSigWallet(
	...
	if timeLock != nil {
		unlockTime := time.Now().Add(timeLock.AsDuration())
...
func (k Keeper) CreatePendingMultiSigTransaction(
	...
	// Check time lock
	if wallet.TimeLocked && time.Now().Before(wallet.UnlockTime.AsTime()) {
	...
	now := timestamppb.New(time.Now())
	expiresAt := timestamppb.New(time.Now().Add(expirationDuration))
...
func (k Keeper) SignMultiSigTransaction(
	...
	// Check expiration
	if time.Now().After(pendingTx.ExpiresAt.AsTime()) {
