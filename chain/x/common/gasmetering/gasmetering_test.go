...
func TestMeteredStoreChargesGas(t *testing.T) {
	...
	ms := NewMeteredStore(ctx, store, DefaultGasConfig())
	gm := ctx.GasMeter()
...
func TestMeteredIteratorHonorsLimit(t *testing.T) {
	...
	ms := NewMeteredStore(ctx, store, config)
...
func TestIterateWithLimit(t *testing.T) {
	...
	err := IterateWithLimit(ctx, store, []byte("k"), 2, DefaultGasConfig(), func(key, value []byte) error {
		return nil
	})
...
func TestConsumeGasHelpers(t *testing.T) {
	...
	ConsumeGasForCrypto(ctx, "hash", config)
	require.Greater(t, gm.GasConsumed(), start)

	current := gm.GasConsumed()
	ConsumeGasForCrypto(ctx, "unknown-op", config)
	require.Greater(t, gm.GasConsumed(), current)

	current = gm.GasConsumed()
	ConsumeGasForValidation(ctx, "signature", config)
	require.Greater(t, gm.GasConsumed(), current)

	current = gm.GasConsumed()
	ConsumeGasForValidation(ctx, "custom", config)
	require.Greater(t, gm.GasConsumed(), current)
}
