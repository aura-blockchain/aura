...
func TestInputValidator(t *testing.T) {
	...
	validAmount := sdkmath.NewInt(100)
...
func TestSafeMath(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, sdkmath.NewInt(150), result)

	// Test safe subtraction
	result, err = sm.SafeSub(a, b)
	require.NoError(t, err)
	require.Equal(t, sdkmath.NewInt(50), result)
	...
	// Test safe division
	result, err = sm.SafeDiv(sdkmath.NewInt(100), sdkmath.NewInt(5))
	require.NoError(t, err)
	require.Equal(t, sdkmath.NewInt(20), result)

	// Test division by zero
	_, err = sm.SafeDiv(sdkmath.NewInt(100), sdkmath.ZeroInt())
	require.Error(t, err)
	require.Equal(t, ErrZeroAmount, err)
}
...
func TestDecimalSafeMath(t *testing.T) {
	...
	a := sdkmath.LegacyNewDec(100)
	b := sdkmath.LegacyNewDec(50)
	result, err := sm.SafeAddDec(a, b)
	require.NoError(t, err)
	require.Equal(t, sdkmath.LegacyNewDec(150), result)

	// Test decimal subtraction
	result, err = sm.SafeSubDec(a, b)
	require.NoError(t, err)
	require.Equal(t, sdkmath.LegacyNewDec(50), result)

	// Test decimal multiplication
	result, err = sm.SafeMulDec(sdkmath.LegacyNewDec(10), sdkmath.LegacyNewDec(5))
	require.NoError(t, err)
	require.Equal(t, sdkmath.LegacyNewDec(50), result)

	// Test decimal division
	result, err = sm.SafeDivDec(sdkmath.LegacyNewDec(100), sdkmath.LegacyNewDec(5))
	require.NoError(t, err)
	require.Equal(t, sdkmath.LegacyNewDec(20), result)

	// Test decimal division by zero
	_, err = sm.SafeDivDec(sdkmath.LegacyNewDec(100), sdkmath.LegacyZeroDec())
	require.Error(t, err)
	require.Equal(t, ErrZeroAmount, err)
}
