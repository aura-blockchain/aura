...
func (k *Keeper) GetIncident(incidentID string) (*types.Incident, error) {
	k.mu.RLock()
	defer k.mu.RUnlock()
...
func (k *Keeper) GetAllIncidents() []*types.Incident {
	k.mu.RLock()
	defer k.mu.RUnlock()
...
func (k *Keeper) RequestChainPause(
	...
	if k.params.PauseRequiredSigners == 1 {
		return k.executeChainPause(requester, pauseLevel, reason, incidentID, duration)
	}
...
func (k *Keeper) ApproveChainPause(
	...
	if uint32(len(k.pauseVotes[pauseRequestID])) >= k.params.PauseRequiredSigners {
		...
		return k.executeChainPause(approver, types.PauseLevelFull, "Multi-sig approved", "", 1*time.Hour)
	}
...
func (k *Keeper) ResumeChain(resumedBy string, reason string) error {
	k.mu.Lock()
	defer k.mu.Unlock()
...
func (k *Keeper) GetChainPauseState() *types.ChainPauseState {
	k.mu.RLock()
	defer k.mu.RUnlock()
	return k.pauseState
}
...
func (k *Keeper) CheckWalletLimit(address string, amount string, currentBalance string) error {
	k.mu.Lock()
	defer k.mu.Unlock()
...
func (k *Keeper) GetWalletLimits(address string) (*types.WalletLimits, error) {
	k.mu.RLock()
	defer k.mu.RUnlock()
...
func (k *Keeper) GetColdStorageConfig() types.ColdStorageConfig {
	k.mu.RLock()
	defer k.mu.RUnlock()
...
func (k *Keeper) CloseIncident(incidentID string, closedBy string) error {
	k.mu.Lock()
	defer k.mu.Unlock()
...
func (k *Keeper) TriggerBackup(backupType string, triggeredBy string) (string, error) {
	k.mu.RLock()
	defer k.mu.RUnlock()
	...
	backupID := fmt.Sprintf("backup-%s-%d", backupType, time.Now().Unix())
...
func (k *Keeper) GetDisasterRecoveryPlan() types.DisasterRecoveryPlan {
	k.mu.RLock()
	defer k.mu.RUnlock()
...
func (k *Keeper) CheckValidatorHealth() error {
	k.mu.Lock()
	defer k.mu.Unlock()
	...
	k.params.BackupValidators.LastHealthCheck = time.Now()
...
func (k *Keeper) GetBackupValidatorConfig() types.BackupValidatorConfig {
	k.mu.RLock()
	defer k.mu.RUnlock()
...
func (k *Keeper) GetCommunicationPlan() types.CommunicationPlan {
	k.mu.RLock()
	defer k.mu.RUnlock()
...
func (k *Keeper) TriggerInsuranceClaim(
	...
	claimID := fmt.Sprintf("claim-%s-%d", incidentID, time.Now().Unix())
...
func (k *Keeper) GetInsuranceIntegration() types.InsuranceIntegration {
	k.mu.RLock()
	defer k.mu.RUnlock()
...
func (k *Keeper) GetParams() types.IncidentResponseParams {
	k.mu.RLock()
	defer k.mu.RUnlock()
	return k.params
}
