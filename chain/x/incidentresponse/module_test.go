...
func setupModule(t *testing.T) (AppModule, *keeper.KeeperKV, sdk.Context, codec.JSONCodec) {
	t.Helper()
	...
	k := keeper.NewKeeperKV(storeKey, binaryCodec)
...
func TestAppModuleBasic_ValidateGenesis(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name:    "empty genesis",
			genesis: json.RawMessage(""),
			wantErr: true, // incidentresponse module requires valid genesis JSON
		},
		{
			name:    "invalid json",
...
func TestNewAppModule(t *testing.T) {
	...
	interfaceRegistry := codectypes.NewInterfaceRegistry()
	cdc := codec.NewProtoCodec(interfaceRegistry)
	k := keeper.NewKeeperKV(storeKey, cdc)
