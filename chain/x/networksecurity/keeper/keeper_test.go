...
func (suite *KeeperTestSuite) TestParams() {
	...
	// Compare field values instead of using require.Equal to avoid protobuf internal field comparison issues
	require.Equal(suite.T(), params.RateLimit.MaxRequestsPerSecond, retrievedParams.RateLimit.MaxRequestsPerSecond)
	require.Equal(suite.T(), params.Connection.MaxInboundConnections, retrievedParams.Connection.MaxInboundConnections)
	require.Equal(suite.T(), params.Mempool.MaxSize, retrievedParams.Mempool.MaxSize)
	require.Equal(suite.T(), params.Reputation.InitialScore, retrievedParams.Reputation.InitialScore)
	require.Equal(suite.T(), params.Gossip.MaxMessageSize, retrievedParams.Gossip.MaxMessageSize)
}
