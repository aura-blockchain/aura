...
func (k Keeper) CheckRateLimit(ctx sdk.Context, peerID string) error {
	...
	if !limiter.Allow() {
		...
		if entry.RequestCount > params.RateLimit.MaxRequestsPerSecond*10 {
			...
			k.BanPeer(ctx, peerID, int64(params.RateLimit.BanDuration.Seconds()), "rate limit violations")
...
func (k Keeper) CleanupExpiredRateLimits(ctx sdk.Context) {
	...
	iterator, err := store.Iterator(types.RateLimitPrefix, sdk.PrefixEndBytes(types.RateLimitPrefix))
	if err != nil {
		return
	}
	...
	for ; iterator.Valid(); iterator.Next() {
		...
		if ctx.BlockTime().Sub(entry.WindowStart) > params.RateLimit.WindowDuration*2 {
