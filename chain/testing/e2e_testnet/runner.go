...
// TestSuite holds the complete test suite results
...
// Runner executes E2E tests against the live testnet
type Runner struct {
...
// RunAll executes all E2E test phases
func (r *Runner) RunAll(ctx context.Context) *TestSuite {
	fmt.Println("=" + strings.Repeat("=", 59))
	fmt.Printf("AURA Testnet E2E Validation - %s\n", time.Now().UTC().Format(time.RFC3339))
...
func (r *Runner) runMultiNodeTests(ctx context.Context) {
	...
	// Check consistency across all validators
...
func (r *Runner) runConsensusTests(ctx context.Context) {
	...
	// 3.2 Multiple validators signing
	...
	// 3.3 Block time reasonable (2-10s)
...
func (r *Runner) runOpsTests(ctx context.Context) {
	...
	// 5.3 Service status
	serviceOutput, err := r.client.RunCommand(ctx, val, "systemctl is-active aurad-val@1 2>/dev/null || systemctl is-active aurad 2>/dev/null")
...
// SaveMarkdown saves the results as a markdown report
func (r *Runner) SaveMarkdown(filepath string) error {
	var sb strings.Builder

	sb.WriteString(fmt.Sprintf("# AURA Testnet E2E Validation Report\n\n"))
	...
	sb.WriteString(fmt.Sprintf("\n---\n*Generated by AURA E2E Test Runner*\n"))
