"""Main client for AURA blockchain."""
...
from .modules.vcregistry import VCRegistryModule
from .modules.bridge import BridgeModule
from .modules.compliance import ComplianceModule
from .modules.confidencescore import ConfidenceScoreModule
from .modules.cryptography import CryptographyModule
from .modules.dataregistry import DataRegistryModule
from .modules.economicsecurity import EconomicSecurityModule
from .modules.identitychange import IdentityChangeModule
from .modules.inclusionroutines import InclusionRoutinesModule
from .modules.monitoring import MonitoringModule
from .modules.networksecurity import NetworkSecurityModule
from .modules.prevalidation import PrevalidationModule
from .modules.privacy import PrivacyModule
from .modules.validatorsecurity import ValidatorSecurityModule
from .modules.walletsecurity import WalletSecurityModule


class AuraClient:
    """Main client for interacting with AURA blockchain."""

    def __init__(self, config: ChainConfig):
        """Initialize the AURA client."""
        self.config = config
        self._wallet: Optional[AuraWallet] = None
        ...
        # Initialize AURA-specific modules
        self.vcregistry = VCRegistryModule(self)
        self.bridge = BridgeModule(self)
        self.compliance = ComplianceModule(self)
        self.confidencescore = ConfidenceScoreModule(self)
        self.cryptography = CryptographyModule(self)
        self.dataregistry = DataRegistryModule(self)
        self.economicsecurity = EconomicSecurityModule(self)
        self.identitychange = IdentityChangeModule(self)
        self.inclusionroutines = InclusionRoutinesModule(self)
        self.monitoring = MonitoringModule(self)
        self.networksecurity = NetworkSecurityModule(self)
        self.prevalidation = PrevalidationModule(self)
        self.privacy = PrivacyModule(self)
        self.validatorsecurity = ValidatorSecurityModule(self)
        self.walletsecurity = WalletSecurityModule(self)

    async def connect_wallet(self, wallet: AuraWallet) -> None:
        """Connect a wallet for signing transactions."""
        self._wallet = wallet
        self._tx_builder = TxBuilder(self, wallet)

    @property
    def wallet(self) -> AuraWallet:
