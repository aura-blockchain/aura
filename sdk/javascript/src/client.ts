...
import { AuraChainConfig } from './types';
import { AuraWallet } from './wallet';
...
export class AuraClient {
  private config: AuraChainConfig;
   ...
   */
  async connectWithWallet(wallet: AuraWallet): Promise<void> {
    const signer = wallet.getSigner();
    this.signingClient = await SigningStargateClient.connectWithSigner(
      this.config.rpcEndpoint,
      signer,
      {
        gasPrice: this.config.gasPrice ? GasPrice.fromString(this.config.gasPrice) : undefined
      }
    );
