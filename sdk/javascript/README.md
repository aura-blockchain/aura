# AURA JavaScript/TypeScript SDK

Official JavaScript/TypeScript SDK for interacting with the AURA blockchain.
...
```typescript
import { AuraWallet } from '@aura-chain/sdk';
...
const wallet = new AuraWallet('paw');
...
```typescript
import { AuraClient, AuraWallet } from '@aura-chain/sdk';
...
const result = await client.bank.send(
  ...
  {
    recipient: 'aura1...',
    amount: '1000000', // 1 AURA (6 decimals)
    denom: 'uaura'
  }
);
...
```typescript
const balance = await client.bank.getBalance(address, 'uaura');
...
await client.bank.multiSend(sender, [
  { address: 'aura1...', amount: '500000' },
  { address: 'aura1...', amount: '300000' }
]);
```
...
// Get specific pool
const pool = await client.dex.getPoolByTokens('uaura', 'uatom');
...
await client.staking.redelegate(delegator, {
  srcValidatorAddress: 'auravaloper1...',
  dstValidatorAddress: 'auravaloper1...',
  amount: '500000'
});

// Withdraw rewards
await client.staking.withdrawRewards(delegator, 'auravaloper1...');
...
```typescript
import { VoteOption } from '@aura-chain/sdk';
...
const result = await client.bank.send(
  sender,
  { recipient: 'aura1...', amount: '1000000' },
  {
    gasLimit: 200000,
    gasPrice: '0.05uaura',
...
- `gasPrice`: Default gas price (default: '0.025uaura')
...
```typescript
import type { Pool, Validator, Proposal, TxResult } from '@aura-chain/sdk';
```
...
```typescript
import { AuraClient } from '@aura-chain/sdk';
...
- GitHub: https://github.com/aura-chain/paw
