...
var AuraClient = class {
   ...
   */
  async connect() {
    const tmClient = await import_tendermint_rpc.Tendermint34Client.connect(this.config.rpcEndpoint);
    this.client = await import_stargate.StargateClient.create(tmClient);
  }
  /**
  ...
  async connectWithWallet(wallet) {
    ...
    this.signingClient = await import_stargate.SigningStargateClient.connectWithSigner(
      this.config.rpcEndpoint,
      signer,
      {
        gasPrice: this.config.gasPrice ? import_stargate.GasPrice.fromString(this.config.gasPrice) : void 0
      }
    );
...
var AuraWallet = class _AuraWallet {
  ...
  async fromMnemonic(mnemonic, hdPath) {
    ...
    const options = hdPath ? { hdPaths: [(0, import_crypto.stringToPath)(hdPath)], prefix: this.prefix } : { prefix: this.prefix };
    this.wallet = await import_proto_signing.DirectSecp256k1HdWallet.fromMnemonic(mnemonic, options);
  }
  /**
   * Get wallet accounts
   */
