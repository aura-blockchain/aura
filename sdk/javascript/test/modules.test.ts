...
describe('DexModule', () => {
  let mockClient: jest.Mocked<AuraClient>;
...
describe('BankModule', () => {
  describe('formatBalance', () => {
    it('should format balance with default decimals', () => {
      ...
      const formatted = bank.formatBalance({ denom: 'uaura', amount: '1000000' });
      expect(formatted).toBe('1.000000 AURA');
    });
    ...
    it('should format balance with custom decimals', () => {
      ...
      const formatted = bank.formatBalance({ denom: 'uaura', amount: '150' }, 2);
      expect(formatted).toBe('1.50 AURA');
    });
  });
});
