...
describe('AuraClient', () => {
  ...
  describe('constructor', () => {
    it('should create client with config', () => {
      const client = new AuraClient(config);
    ...
    it('should use default prefix and gas settings', () => {
      ...
      expect(clientConfig.prefix).toBe('aura');
      expect(clientConfig.gasPrice).toBe('0.025uaura');
    ...
    it('should allow custom prefix and gas settings', () => {
      ...
      const client = new AuraClient(customConfig);
      const clientConfig = client.getConfig();

      expect(clientConfig.prefix).toBe('custom');
      expect(clientConfig.gasPrice).toBe('0.05uaura');
