...
// Config holds the AURA client configuration
type Config struct {
	RPCEndpoint   string
	GRPCEndpoint  string
	ChainID       string
...
func (c *Client) SignAndBroadcast(ctx context.Context, fromName string, msgs ...sdk.Msg) (*sdk.TxResponse, error) {
	...
	txBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewCoin("uaura", math.NewInt(5000))))
	...
	// Generate signature bytes to sign
	...
	// Sign the bytes using keyring
	sigBytes, _, err := c.keyring.SignByAddress(fromAddr, bytesToSign, signing.SignMode_SIGN_MODE_DIRECT)
	if err != nil {
...
// ClientContext wraps the SDK client context with additional fields
type ClientContext struct {
	GRPCClient *grpc.ClientConn
	ChainID    string
	SDKContext client.Context
}

// GetClientContext returns the client context with GRPC connection
func (c *Client) GetClientContext() ClientContext {
	return ClientContext{
		GRPCClient: c.grpcConn,
		ChainID:    c.config.ChainID,
		SDKContext: c.clientCtx,
	}
}
