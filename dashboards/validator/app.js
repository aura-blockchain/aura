// AURA Validator Dashboard - Main Application
...
class ValidatorDashboard {
    ...
    loadValidators() {
        const stored = localStorage.getItem('aura_validators');
        return stored ? JSON.parse(stored) : [];
    }

    saveValidators() {
        localStorage.setItem('aura_validators', JSON.stringify(this.validators));
    }
    ...
    async addValidator() {
        ...
        if (!address.startsWith('auravaloper')) {
    ...
    formatAmount(amount) {
        if (!amount) return '0 AURA';
        const value = parseFloat(amount);
        if (value >= 1000000) {
            return `${(value / 1000000).toFixed(2)}M AURA`;
        } else if (value >= 1000) {
            return `${(value / 1000).toFixed(2)}K AURA`;
        }
        return `${value.toFixed(2)} AURA`;
    }
